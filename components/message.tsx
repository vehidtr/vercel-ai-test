...
export const PreviewMessage = ({
  ...
  const question = extractQuestionAndAnswer(messages, message?.id);

  function isGithubRepoUrl(url: string) {
    return githubRepoRegex.test(url);
  }

  function extractLinks(message: any) {
    return message.match(urlRegex) || [];
  }

  async function fetchRepoDetailsFromUrl(url: string) {
    ...
    const fetchGithubLinks = async () => {
      ...
      if (links?.length) {
        ...
        if (parsedLinks.length > 0) {
          const details: GithubLink[] = [];
          for (const link of parsedLinks) {
            try {
              const detail = await fetchRepoDetailsFromUrl(link);
              if (detail) {
                details.push(detail);
              }
            } catch (error) {
              console.error(`Error fetching details for link ${link}:`, error);
            }
          }

          setGithubLinks(details);
        }
      }
    };
